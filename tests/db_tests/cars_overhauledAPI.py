...
def create_database():
    ...
    cursor.execute(
        "CREATE TABLE cust_order (orderid INTEGER PRIMARY KEY NOT NULL, description TEXT NOT NULL, customer INTEGER "
        "NOT NULL, FOREIGN KEY(customer) REFERENCES customer(id)) "
    )
...
def query_database():
    ...
    cursor.execute(
        ...
        "customer.id WHERE customer.first_name = 'Rebecca' "
    )
